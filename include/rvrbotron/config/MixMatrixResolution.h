...
// one draw per matrix cell (row, column); the matrix is shared across
// Diffusion Steps rather than per-step, so the derivation does not use a
// step index. Not itself orthogonal -- the input to Householder QR
// orthogonalization.
[[nodiscard]] std::vector<double> fillRandomOrthogonalSeed(
    std::uint32_t channels, std::uint64_t seed);
...
// fillRandomOrthogonalSeed(channels, seed) followed by
// householderQrOrthogonalize(...).
[[nodiscard]] std::optional<std::vector<double>>
resolveRandomOrthogonalMatrix(std::uint32_t channels, std::uint64_t seed);
