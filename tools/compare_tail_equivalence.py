...
"""Compare compact tail-equivalence summaries (see
extract_tail_equivalence.py) captured on different platforms, grouped by
sample precision, against one baseline per group.

Mirrors compare_diffusion_equivalence.py. Whether a band's T20/T30 fit
exists at all, the octave-band count, and the decay-envelope monotonicity
flag are discrete facts about the committed CI tracer's decay curve, not
continuous quantities a headroom tolerance suits -- see
docs/adr/0001-cross-platform-reproducibility.md -- so they are compared
for exact equality unconditionally, on the same footing as diffusion's
structural Echo-path count. Every measured metric -- including each band's
own T20/T30 RT60 estimate -- is compared against the absolute tolerance
committed in a tolerances file (tools/tail_tolerances_v1.json by default).
"""
...
def structural_metrics(entry):
    ...
    in a tolerances document: the octave-band count (fixed by
    OCTAVE_BAND_CENTERS_HZ and the sample rate), whether each band's T20/T30
    fit exists, and the decay-envelope monotonicity flag."""
    ...
    yield "decayEnvelopeMonotonic", entry["decayEnvelopeMonotonic"]
...
def _compare_metric(precision, platform, name, expected, actual, tolerance):
    if actual is _MISSING:
        # A band-count (or similarly keyed) divergence between platforms
        # means some per-band metric the baseline reports has no candidate
        # counterpart at all -- report that plainly as its own failing row
        # rather than letting a KeyError abort the whole comparison.
        return {
            "precision": precision,
            "platform": platform,
            "metric": name,
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            "Compare compact tail-equivalence summaries across platforms, "
            "grouped by sample precision, against committed tolerances."
        )
    )
    ...
    parser.add_argument(
        "--tolerances",
        type=Path,
        default=Path(__file__).parent / "tail_tolerances_v1.json",
    )
...
def main():
    ...
    try:
        ...
        print(f"tail-equivalence comparison failed: {error}", file=sys.stderr)
        return 1
