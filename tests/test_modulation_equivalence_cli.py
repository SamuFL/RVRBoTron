...
REQUIRED_METRIC_KEYS = {
    ...
    "decayRelativeError",
    "decayTiltSlopeRatioPerOctave",
    "decayTiltInterceptRatio",
    "boundedEnergySlopeDbPerSegment",
    "coherentPitchMovementBroadbandMeanMagnitude",
    "coherentPitchMovementMagnitude",
    "coherentPitchMovementRelativeMagnitude",
...
def main():
    ...
    # Feedback Loop Modulation (issue #94 wants the cross-platform CI
    # tracer to actually exercise Modulation).
    ...
    if "perChannelRt60Sec" in dumped or "segmentEnergiesDb" in dumped:
        raise AssertionError(
            "compact modulation-equivalence artifact leaked bulky per-sample "
            "evidence"
        )
    if equivalence["nonFiniteSampleCount"] != 0:
        raise AssertionError(f"unexpected non-finite samples: {equivalence}")
    if equivalence["modulationPresent"] is not True:
        raise AssertionError(f"modulationPresent was not carried through: {equivalence}")
    if equivalence["activeModulationCount"] != 1:
        raise AssertionError(f"unexpected activeModulationCount: {equivalence}")
    ...
    if missing_analysis.returncode == 0:
        raise AssertionError("extraction succeeded without modulation-v1.json")
    ...
    # modulationPresent is likewise structural: a platform diverging on it
    # must fail unconditionally, never silently folded into a
    # tolerance-checked numeric comparison.
    modulation_flag_mismatch = json.loads(equivalence_b.read_text())
    modulation_flag_mismatch["modulationPresent"] = False
    modulation_flag_mismatch_path = (
        workspace / "equivalence-modulation-flag-mismatch.json"
    )
    modulation_flag_mismatch_path.write_text(json.dumps(modulation_flag_mismatch))
    modulation_flag_mismatched = run(
        sys.executable, comparator, equivalence_a, modulation_flag_mismatch_path
    )
    if modulation_flag_mismatched.returncode != 1:
        raise AssertionError(
            f"modulationPresent mismatch was not reported as a failure: "
            f"{modulation_flag_mismatched.stdout}"
        )
    if "modulationPresent" not in modulation_flag_mismatched.stdout:
        raise AssertionError(
            f"modulationPresent mismatch did not name the metric: "
            f"{modulation_flag_mismatched.stdout}"
        )

    # The bounded-energy trend sign is likewise structural.
    trend_mismatch = json.loads(equivalence_b.read_text())
    trend_mismatch["boundedEnergy"]["negativeTrend"] = False
    ...
    if trend_mismatched.returncode != 1:
        raise AssertionError(
            f"bounded-energy trend mismatch was not reported as a failure: "
            f"{trend_mismatched.stdout}"
        )
    if "boundedEnergy.negativeTrend" not in trend_mismatched.stdout:
        raise AssertionError(
    ...
    numeric_breach["outputCorrelation"]["meanAbsoluteOffDiagonal"] += 1e-6
    ...
    if "outputCorrelationMeanAbsoluteOffDiagonal" not in breached.stdout:
        raise AssertionError(
    ...
    # comparison while allowing a separately evidenced arm64-class delta.
    ...
    macos_x86["outputCorrelation"]["meanAbsoluteOffDiagonal"] += 2e-6
    ...
    windows_x86["outputCorrelation"]["meanAbsoluteOffDiagonal"] += 4e-6
