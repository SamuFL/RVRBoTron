...
void readAt(std::ifstream& input,
  ...
  if (!input) {
    throw HarnessError(
        ErrorCategory::unsupportedAudio, "truncated input WAV");
  }
}
...
void validateWavHeader(const std::filesystem::path& path) {
  ...
  if (!input) {
    throw HarnessError(
        ...
        "could not open input WAV: " + path.string());
  }
  const auto end = input.tellg();
  if (end < 0) {
    throw HarnessError(
        ErrorCategory::ioFailure,
        "could not inspect input WAV: " + path.string());
  }
  const auto fileSize = static_cast<std::uint64_t>(end);
  if (fileSize < 12) {
    throw HarnessError(
        ErrorCategory::unsupportedAudio, "malformed input WAV");
  }
  ...
  if (channels == 0 || channels > 2) {
    throw HarnessError(
        ErrorCategory::unsupportedAudio,
  ...
  if (!supportedPcm && !supportedFloat) {
    throw HarnessError(
        ErrorCategory::unsupportedAudio, "unsupported WAV encoding");
  }
...
std::size_t WavReader::readFrames(dsp::Sample* interleaved,
  ...
  if (frameCount >
    ...
    throw HarnessError(
        ErrorCategory::internalProcessingFailure,
        "WAV processing block is too large");
  }
  ...
  for (std::size_t index = 0; index < sampleCount; ++index) {
    ...
    if (!std::isfinite(interleaved[index])) {
      throw HarnessError(
          ErrorCategory::unsupportedAudio, "non-finite input sample");
    }
  }
...
WavWriter::WavWriter(const std::filesystem::path& path,
  ...
  if (!drwav_init_file_write(
    ...
    throw HarnessError(
        ErrorCategory::ioFailure,
        "could not create output WAV: " + path.string());
  }
  implementation_->open = true;
}
...
void WavWriter::writeFrames(const dsp::Sample* interleaved,
  ...
  if (written != frameCount) {
    throw HarnessError(
        ErrorCategory::ioFailure,
        "could not write all output WAV frames");
  }
}
...
void WavWriter::close() {
  ...
  const auto result = drwav_uninit(&implementation_->wav);
  implementation_->open = false;
  if (result != DRWAV_SUCCESS) {
    throw HarnessError(
        ErrorCategory::ioFailure, "could not finalize output WAV");
  }
}
